...
<head>
    ...
    <title>Ramak Slagerij - Voorraad Bewerken</title>
    @vite(['resources/css/app.css'])
</head>
<body class="bg-white flex flex-col min-h-screen">
<div class="container mx-auto py-8">
    <h2 class="text-2xl font-semibold text-slagerij-green mb-6">Voorraad Bewerken</h2>
    <form action="{{ route('stock.update', $stock->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <div class="flex items-center space-x-2">
                <input type="number" name="quantity" id="quantity" class="w-full px-4 py-2 border rounded-md" value="{{ old('quantity', $stock->quantity) }}" required min="0" step="0.01">
        ...
        <div>
            ...
            <input type="date" name="date" id="date" class="w-full px-4 py-2 border rounded-md" value="{{ old('date', $stock->date ? \Carbon\Carbon::parse($stock->date)->format('Y-m-d') : now()->toDateString()) }}">
            @error('date')
            <div class="text-red-600 text-sm">{{ $message }}</div>
            @enderror
        </div>
